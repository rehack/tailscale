...
func TestDoChanContext(t *testing.T) {
	...
	t.Run("PropagatesValues", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		var g Group[string, int]
		ch := g.DoChanContext(ctx, "key", func(_ context.Context) (int, error) {
			gotVal, ok := ctx.Value(key).(string)
			if !ok {
				t.Fatalf("expected value to be present in context")
			}
			if gotVal != value {
				t.Fatalf("unexpected value; got %q, want %q", gotVal, value)
			}
			return 1, nil
		})
		ret := <-ch
			...
			t.Run(fmt.Sprintf("NumWaiters=%d", n), func(t *testing.T) {
				...
				fn := func(ctx context.Context) (int, error) {
					...
					case <-ctx.Done():
						return 0, ctx.Err()
					case <-trigger:
						t.Fatal("unexpected trigger; want all callers to cancel")
