...
func run(ctx context.Context, hostname string, debugPort int, siteID uint16, v4PfxStr string, verboseTSNet bool) {
	...
	ts.RegisterFallbackTCPHandler(c.handleTCPFlow)
	...
	pc, err := ts.ListenPacket("udp4", dnsAddr.String()+":53")
	if err != nil {
		log.Fatalf("failed listening on port 53: %v", err)
	}
	defer pc.Close()
	log.Printf("Listening for DNS on %s", pc.LocalAddr())
	c.serveDNS(pc)
}
...
func (c *connector) serveDNS(pc net.PacketConn) {
	for {
		...
		go c.handleDNS(pc, remoteAddr, buf[:n])
	}
}
...
func (c *connector) handleDNS(pc net.PacketConn, remoteAddr net.Addr, buf []byte) {
...
func (ps *perPeerState) domainForIP(ip netip.Addr) (string, bool) {
	ps.mu.Lock()
	defer ps.mu.Unlock()
...
func (ps *perPeerState) ipForDomain(domain string) []netip.Addr {
